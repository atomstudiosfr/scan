...
def main():
    for site in config.sites:
        main_page_html = fetch_page(site.site_url)
        if main_page_html:
            ...
            for manga_url, manga_title in manga_links:
                ...
                if site.check_new_chapters and os.path.exists(manga_dir):
                    ...
                    if chapter_page_html:
                        ...
                        webp_images = save_images(images, chapter_dir, site.overwrite)
                        ...
                        update_manga_scans_json(site, manga_dir, str(next_chapter), cleaned_images)
                    continue
                ...
                if manga_page_html:
                    ...
                    if cover_url:
                        ...
                        save_image(cover_url, cover_path, site.overwrite)

                    chapters = parse_manga_page(manga_page_html)
                    chapters = clean_chapters(chapters)  # Clean chapters before processing
                    for chapter_url in sorted(chapters, key=extract_chapter_number):
